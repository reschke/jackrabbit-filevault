...
public class DocViewImporter implements DocViewParserHandler {
    ...
    @Override
    public void startDocViewNode(String parentNodePath, DocViewNode2 docViewNode) throws IOException, RepositoryException {
        stack.addName(docViewNode.getName());
        ...
        if (node == null) {
            ...
            try {
                // is policy node?
                if (docViewNode.getPrimaryType().filter(type -> aclManagement.isACLNodeType(type)).isPresent()) {
                ...
                } else if (userManagement != null && docViewNode.getPrimaryType().filter(type -> userManagement.isAuthorizableNodeType(type)).isPresent()) {
    ...
    @Override
    public void endDocViewNode(String parentNodePath, DocViewNode2 docViewNode) throws IOException, RepositoryException {
    ...
    private void registerBinary(Artifact a, String rootPath)
        ...
        if (a.getType() == ArtifactType.FILE && a instanceof PropertyValueArtifact) {
            ...
            Map<String, BlobInfo> infoSet = binaries.get(parentPath);
            if (infoSet == null) {
                infoSet = new HashMap<String, BlobInfo>();
                binaries.put(parentPath, infoSet);
            }
            BlobInfo info = infoSet.get(name);
            if (info == null) {
    ...
    private StackElement addNode(DocViewNode2 docViewNode) throws RepositoryException, IOException {
        ...
        boolean isCheckedIn = "false".equals(docViewNode.getPropertyValue(NameConstants.JCR_ISCHECKEDOUT));

        // create or update node
        boolean isNew = existingNode == null;
        if (isNew) {
            // workaround for bug in jcr2spi if mixins are empty
            if (docViewNode.getProperty(NameConstants.JCR_MIXINTYPES) == null) {
    ...
    private @NotNull Node createNewNode(Node parentNode, DocViewNode2 ni)
        ...
        try {
            ...
            attrs.addAttribute(Name.NS_SV_URI, "name", "sv:name", "CDATA", npResolver.getJCRName(ni.getName()));
            ...
            if (ni.getIndex() > 0 && ni.getIdentifier() == null) {
                ...
                if (mix == null) {
                    ...
                    if (addMixRef) {
                        List<String> mixinValues = mix.getStringValues();
            ...
            // add the properties
            for (DocViewProperty2 p : preprocessedProperties) {
                if (p != null && p.getStringValue() != null) {
                    // only pass 'protected' properties to the import
                    if (PROTECTED_PROPERTIES.contains(p.getName())) {
                        attrs = new AttributesImpl();
                        attrs.addAttribute(Name.NS_SV_URI, "name", "sv:name", "CDATA", npResolver.getJCRName(p.getName()));
    ...
    private Node getNodeByIdOrName(@NotNull Node currentNode, @NotNull DocViewNode2 ni, boolean isUuidNewlyAssigned) throws RepositoryException {
        ...
        if (ni.getIdentifier().isPresent() && !isUuidNewlyAssigned) {
            try {
                node = currentNode.getSession().getNodeByUUID(ni.getIdentifier().get());
            } catch (RepositoryException e) {
        ...
        if (node == null) {
            try {
                ...
                log.warn("Newly created node not found by label {}: {}", currentNode.getPath() + "/" + ni.getSnsAwareName(), e.toString());
            }
        }
        if (node == null) {
            try {
